...
from shared.capital_manager import calculateLots, load_capital, save_capital, get_lot_size, calculate_total_charges
...
def findStrikePriceATM():
    ...
    atmCE = f"{stock}{intExpiry}C{closest_Strike_CE}"
    atmPE = f"{stock}{intExpiry}P{closest_Strike_PE}"
...
def exitPosition(atmCE, atmPE, qty):
    ...
    while traded == "No":
        ...
        try:
            ...
            if (ltp_ce > ceSL or (dt.hour >= 15 and dt.minute >= 15)):
                placeOrder(atmCE, "BUY", qty, "MARKET", 0, "regular")
                ce_exit_price = ltp_ce
                ce_exit_reason = "TIME_EXIT" if dt.hour >= 15 and dt.minute >= 15 else "SL_HIT"
                traded = "CE"
            elif (ltp_pe > peSL or (dt.hour >= 15 and dt.minute >= 15)):
                placeOrder(atmPE, "BUY", qty, "MARKET", 0, "regular")
                pe_exit_price = ltp_pe
                pe_exit_reason = "TIME_EXIT" if dt.hour >= 15 and dt.minute >= 15 else "SL_HIT"
                traded = "PE"
            else:
                time.sleep(1)
        except: time.sleep(1)
    ...
    if traded == "CE":
        while traded == "CE":
            dt = datetime.datetime.now()
            ltp_pe = float(getLTP("NFO",atmPE))
            if ltp_pe <= (pe_entry_price * 0.30) or (dt.hour >= 15 and dt.minute >= 15) or ltp_pe > pe_entry_price:
                placeOrder(atmPE, "BUY", qty, "MARKET", 0, "regular")
                pe_exit_price = ltp_pe
                pe_exit_reason = "TRAILING_EXIT" if ltp_pe <= (pe_entry_price*0.3) else ("TIME_EXIT" if dt.hour >= 15 else "SL_HIT")
                traded = "Close"
        ...
        while traded == "PE":
            dt = datetime.datetime.now()
            ltp_ce = float(getLTP("NFO",atmCE))
            if ltp_ce <= (ce_entry_price * 0.30) or (dt.hour >= 15 and dt.minute >= 15) or ltp_ce > ce_entry_price:
                placeOrder(atmCE, "BUY", qty, "MARKET", 0, "regular")
                ce_exit_price = ltp_ce
                ce_exit_reason = "TRAILING_EXIT" if ltp_ce <= (ce_entry_price*0.3) else ("TIME_EXIT" if dt.hour >= 15 else "SL_HIT")
                traded = "Close"
