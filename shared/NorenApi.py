...
class NorenApi:
    ...
    def __init__(self):
        self.__service_config['host'] = 'https://api.shoonya.com/NorenWClientAPI'
        self.__service_config['websocket_endpoint'] = 'wss://api.shoonya.com/NorenWSAPI'
    ...
    def login(self, userid, password, twoFA, vendor_code, api_secret, imei):
        ...
        try:
            res = self.__session.post(url, data=payload, timeout=20)
        except Exception as e:
            return {
                'stat': 'Not_Ok',
                'emsg': f'Network error while calling Shoonya login API: {e}'
            }
        ...
        try:
            ...
            return {
                'stat': 'Not_Ok',
                'emsg': (
                    f'Non-JSON response from Shoonya login API (HTTP {res.status_code}). '
                    'This is commonly caused by IP whitelist/access restrictions. '
    ...
    def web_login(self, userid, password, twoFA):
        """Login via the web platform endpoint (no personal API key needed).

        Uses the platform-level vendor code and appkey derived from a
        fixed secret, so this method is immune to personal API-key expiry.
        """
        _WEB_ENDPOINT = 'https://api.shoonya.com/NorenWClientWeb/QuickAuth'
        _WEB_SECRET_BYTES = [83, 50, 97, 114, 110, 46, 27, 93]

        pwd = hashlib.sha256(password.encode('utf-8')).hexdigest()
        secret = ''.join(chr(b + i) for i, b in enumerate(_WEB_SECRET_BYTES))
        appkey = hashlib.sha256(f'{userid}|{secret}'.encode('utf-8')).hexdigest()
        ...
        payload = 'jData=' + json.dumps(values)
        reportmsg('WebLogin Req:' + payload)

        try:
            res = self.__session.post(_WEB_ENDPOINT, data=payload, timeout=20)
        except Exception as e:
            return {
                'stat': 'Not_Ok',
                'emsg': f'Network error during web login: {e}',
            }

        reportmsg('WebLogin Reply:' + res.text)
        ...
        try:
            ...
            return {
                'stat': 'Not_Ok',
                'emsg': (
                    f'Non-JSON response from web login (HTTP {res.status_code}). '
                    f'Response preview: {body_preview}'
                ),
            }
